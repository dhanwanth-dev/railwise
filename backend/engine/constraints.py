...
def evaluate_constraints(
    ...
    *,
    ...
    # ── 3–11. Compliance lattice (toggleable for ablation) ───────────────────
    if not cfg.use_compliance_blocks:
        # Minimal mode: only issuer health + mandate vitality if those toggles are on
        pass
    else:
    ...
    # ── 12. Issuer systemic failure ─────────────────────────────────────────
    if cfg.use_issuer_health:
        monitor = get_monitor()
